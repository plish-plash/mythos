...
pub use bootloader_api::info::PixelFormat;
...
pub type FrameBuffer = Buffer<&'static mut [u8]>;

impl FrameBuffer {
    pub fn from_framebuffer(framebuffer: &'static mut bootloader_api::info::FrameBuffer) -> Self {
        let info = framebuffer.info();
        Buffer {
            width: info.width as u32,
            height: info.height as u32,
            stride: info.stride,
            data: framebuffer.buffer_mut(),
        }
    }
}

pub type VecBuffer = Buffer<Vec<u8>>;
...
impl VecBuffer {
    pub fn alloc(context: &GraphicsContext, width: u32, height: u32) -> Self {
...
pub fn load_system_font(context: &GraphicsContext, color: [u8; 3]) {
    let image = Image {
        width: 128,
        height: 64,
        format: ImageFormat::Mask(color, [0, 0, 0]),
